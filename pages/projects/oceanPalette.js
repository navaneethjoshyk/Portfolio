...
const AutoScrollSection = ({ title, images }) => {
  return (
    <div className="mb-16">
      <h3 className="text-gray-500 uppercase text-xs mb-6 tracking-[0.3em] font-medium">{title}</h3>
      <div className="relative flex overflow-hidden group">
        <div className="absolute inset-y-0 left-0 w-24 bg-gradient-to-r from-[#000] to-transparent z-10"></div>
        <div className="absolute inset-y-0 right-0 w-24 bg-gradient-to-l from-[#000] to-transparent z-10"></div>

        <div className="flex space-x-10 animate-loop-scroll group-hover:paused w-max">
          {images.map((img, index) => (
            <div key={index} className="w-[350px] laptop:w-[550px] aspect-[4/3] rounded-[2rem] overflow-hidden border border-white/5 bg-[#0D0D0D]">
              <img src={img} className="w-full h-full object-cover opacity-80 hover:opacity-100 transition-opacity duration-500" alt={`${title} ${index}`} />
            </div>
          ))}
          {images.map((img, index) => (
            <div key={`dup-${index}`} className="w-[350px] laptop:w-[550px] aspect-[4/3] rounded-[2rem] overflow-hidden border border-white/5 bg-[#0D0D0D]">
              <img src={img} className="w-full h-full object-cover opacity-80 hover:opacity-100 transition-opacity duration-500" alt={`${title} dup ${index}`} />
            </div>
          ))}
        </div>
      </div>
